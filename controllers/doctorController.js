...
exports.getDoctorSideBarDetail = async (req, res) => {
  try {
    ...
    const [result] = await conn.query(
      `select profile,concat(fname, " ",lname) as name,email from users where role_id = ? and users.id = ?`,
      [2, id]
    );
    res.json(result);
  } catch (error) {
...
exports.doctorPaymentData = async (req, res) => {
  try {
    const { id } = req.params;
    const [result] = await conn.query(
      `select concat(fname ," ", lname) as name,payment_amount, concat(start_time," to ",end_time) as SlotTime, date,payments.created_at as PaymentDate from payments inner join time_slots on payments.slot_id = time_slots.id inner join users on payments.patient_id = users.id where payments.doctor_id=?;`,
      [id]
    );
    res.json(result);
  } catch (error) {
...
exports.doctorReviewData = async (req, res) => {
  try {
    ...
    let [data] = await conn.query(
      `select concat(fname," ",lname) as name, rating_and_reviews.rating, rating_and_reviews.review ,rating_and_reviews.created_at as date from rating_and_reviews inner join users on rating_and_reviews.patient_id = users.id where rating_and_reviews.doctor_id = ?`,
      [id]
    );
